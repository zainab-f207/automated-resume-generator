...
{
    public partial class ExportDraftResume : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack && Request.QueryString["FormattedResumeID"] != null && Request.QueryString["format"] != null && Request.QueryString["TemplateID"] != null)
            {
                string formattedresumeId = Request.QueryString["FormattedResumeID"];
                if (!string.IsNullOrEmpty(formattedresumeId))
                {
                    string format = Request.QueryString["format"];
                    if (!int.TryParse(Request.QueryString["TemplateID"], out int templateId) || templateId == 0)
                    {
                    ...
                    DraftResumeData resume = GetResumeData(formattedresumeId);
                ...
                {
                    Response.Write($"Error: Invalid Resume ID.");
                    return;
                }
            }
        }
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            string resumeId = Convert.ToString(Request.QueryString["FormattedResumeID"]);


            DraftResumeData resumeData = GetResumeData(resumeId);
        ...
        private DraftResumeData GetResumeData(string resumeId)
        {
            ...
            DraftResumeData resume = new DraftResumeData();
            ...
            {
                string query = "SELECT * FROM SaveDraftResumes WHERE FormattedResumeID = @FormattedResumeID";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@FormattedResumeID", resumeId.ToString());
